...
env_files=$($GIT_BIN diff --cached --name-only --diff-filter=AM | grep -E '(^|/)\.env(\.|$)' | grep -vE '\.example$')
if [ -n "$env_files" ]; then
  echo ""
...
pattern='AIza[0-9A-Za-z_-]{35}|\bsk-[a-zA-Z0-9_-]{20,}|ghp_[A-Za-z0-9]{36}|github_pat_[A-Za-z0-9_]{22,}|xox[bp]-[0-9A-Za-z-]{10,}|AKIA[0-9A-Z]{16}'
hits=$($GIT_BIN diff --cached -U0 --diff-filter=AM | grep -E '^\+' | grep -EI "$pattern" | head -5)
if [ -n "$hits" ]; then
  echo ""
