...
- **Control Flow & Pipeline Continuation:** In `core/flatten.sh`, when `USE_MANIFEST` is false, `index.mjs` generates `$FULL_PACK` directly. Script execution then continues directly through caller tasks and downstream `core/chunk.sh` processing without calling `exit 0` prematurely.
